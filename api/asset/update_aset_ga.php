...
// RBAC: All non-ADMIN roles may update assets (IT and GA). ADMIN is monitoring/approval only.
...
if ($input && isset($input['id'])) {
    ...
    if (!empty($attachment) && $updateKondisi) {
        $query = "UPDATE aset_ga SET nama_barang = ?, serial_number = ?, asset_class = ?, pic = ?, area = ?, location_note = ?, utilisasi = ?, date_of_entry = ?, attachment = ?, kondisi = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssssssssi", $nama_barang, $serial_number, $asset_class, $pic, $area, $location_note, $utilisasi, $date_of_entry, $attachment, $kondisi, $id);
        ...
        $query = "UPDATE aset_ga SET nama_barang = ?, serial_number = ?, asset_class = ?, pic = ?, area = ?, location_note = ?, utilisasi = ?, date_of_entry = ?, kondisi = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssssssssi", $nama_barang, $serial_number, $asset_class, $pic, $area, $location_note, $utilisasi, $date_of_entry, $kondisi, $id);
    } else {
